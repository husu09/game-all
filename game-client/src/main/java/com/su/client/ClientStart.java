...
public class ClientStart {
	 ...
	 *            显示协议参数的面板
	 */
	public void loadProto(JComponent leftRight, JPanel leftBelow) {
		scanProto("com.su.proto");
		ProtoButtonHandler pbh = new ProtoButtonHandler(leftBelow);
		ProtoContext protoContext = ProtoContext.getInstance();
		for (String s : protoContext.getClassMap().keySet()) {
			if (s.contains("Resp")) continue;
			JButton tB = new JButton(s);
	...
	public static Class<?> getClass(String simpleName) {
		ProtoContext protoContext = ProtoContext.getInstance();
		return protoContext.getClass(simpleName);
	}
}
